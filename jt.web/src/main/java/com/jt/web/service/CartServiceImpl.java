...
public class CartServiceImpl implements CartService {
	...
	public List<Cart> findCartByUserId(Long userId) {
		...
		try {
			...
			/** 数据data的数据类型是Object ,需要转化为LIst集合,转化会失败
				objectMapper.readValue(resultJSON, SysResult.class);
				通过ObjectMapper.readTree 直接获取json的属性 
			要求:JSON串必须是对象的JSON{id:1,name:tom}的格式, 
				 List/array的JSON [value1,value2]获取不到数据
			 */
			//{id:1,name:tom}
	...
	public void updateCartNum(Cart cart) {
		//定义uri http://cart.jt.com/cart/update/num/{userId}/{itemId}/{num}
		String uri = 
